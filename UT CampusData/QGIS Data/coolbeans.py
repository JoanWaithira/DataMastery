...
alldata_times = pd.read_csv(alldata_path, on_bad_lines='skip', delimiter=';', low_memory=False)
...
# Filter the data by season (winter and summer)
cooling_A_winter = cooling_A[alldata_times['season'] == 'winter']  # Assuming 'wet' is winter
cooling_A_summer = cooling_A[alldata_times['season'] == 'summer']  # Assuming 'dry' is summer

# Plot winter (wet season) cooling effects
plt.figure(figsize=(10, 6))
for location in locations:
    plt.plot(cooling_A_winter['chrontimes'], cooling_A_winter[f'deltaT_{location}'], label=location)

plt.title('Cooling Effect in Winter (Wet Season)')
...
# Plot summer (dry season) cooling effects
plt.figure(figsize=(10, 6))
for location in locations:
    plt.plot(cooling_A_summer['chrontimes'], cooling_A_summer[f'deltaT_{location}'], label=location)

plt.title('Cooling Effect in Summer (Dry Season)')
plt.xlabel('Time of Day')
plt.ylabel('Delta T (C)')
plt.legend(loc='upper right', fontsize='small')
plt.grid(True)
